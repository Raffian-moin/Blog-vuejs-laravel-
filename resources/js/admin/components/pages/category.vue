		...
		<div class="content">
			<div class="container-fluid">	
				...
				<div class="_1adminOverveiw_table_recent _box_shadow _border_radious _mar_b30 _p20">
					<p class="_title0">Category<Button size="small" @click="addModal=true"><Icon type="md-add-circle"/>Add Category</Button></p>
					...
					<div class="_overflow _table_div">
						<table class="_table">
							...
							<tr v-for="(tag,index) in tags" :key="index" v-if="tags.length">
								...
								<td>
									<Button type="info" ghost @click="showEditModal(tag,index)">Show</Button>
									<Button type="error" ghost @click="deleteTag(tag.id,index)">Delete</Button>
								</td>
							</tr>
						...
						>
							...
							<Input v-model="data.tagName" size="large" placeholder="Add your category" />
							<div slot="footer">
								<Button type="default" size="small" @click="addModal=false">Cancel</Button>
								<Button type="info" size="small" @click="addTag()"
								:loading="isAdding" :disabled="isAdding">{{isAdding?'Adding..':'Add Category'}}</Button>
							</div>
						</Modal>
						...
						>
							...
							<div slot="footer">
								...
								<Button type="info" size="small" @click="editTag(editData.id)"
								:loading="isAdding" :disabled="isAdding">{{isAdding?'Adding..':'Update tag'}}</Button>
							</div>
						</Modal>
					</div>
				</div>
...
export default {
	...
	methods:{
		...
		async editTag(id){
			...
			const response=await this.callApi('put','/admintag/'+id,this.editData);
			if(response.status==200){
